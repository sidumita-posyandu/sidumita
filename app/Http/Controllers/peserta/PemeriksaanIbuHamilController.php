...
class PemeriksaanIbuHamilController extends Controller
{
    public function index(Request $request){
        ...
        ->get('http://127.0.0.1:8080/api/me/ibu-hamil')->json();
    ...
    public function detIbuHamil(Request $request, $id)
    {
        $response = Http::accept('application/json')
        ->withToken($request->session()->get('token'))
        ->get('http://127.0.0.1:8080/api/pemeriksaan-ibuhamil/ibuhamil/'.' '.$id)->json();
        $det_ibu_hamil = $response['data'][0];

        $response2 = Http::accept('application/json')
        ->withToken($request->session()->get('token'))
        ->get('http://127.0.0.1:8080/api/detail-keluarga/'.' '.$det_ibu_hamil['ibu_hamil']['detail_keluarga_id'])->json();
        $ibu_hamil = $response2['data'][0];

        $response4 = Http::accept('application/json')
        ->withToken($request->session()->get('token'))
        ->get('http://127.0.0.1:8080/api/dusun/'.' '.$ibu_hamil['keluarga']['dusun_id'])->json();
